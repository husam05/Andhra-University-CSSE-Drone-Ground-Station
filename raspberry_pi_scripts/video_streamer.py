...
class VideoStreamer:
    def __init__(self, config_file: str = 'config.json') -> None:
        ...
        self.gstreamer_process: Optional[subprocess.Popen] = None
    ...
    def detect_camera_type(self) -> Optional[str]:
    ...
    def build_gstreamer_pipeline(self, camera_type: str) -> Optional[List[str]]:
        ...
        if camera_type == 'rpi_camera':
            pipeline = [
                ...
                '!', 'videoconvert',
                '!', 'x264enc', f'bitrate={bitrate // 1000}',
                'speed-preset=ultrafast', 'tune=zerolatency',
                '!', 'rtph264pay', 'config-interval=1', 'pt=96',
                '!', 'udpsink', f'host={ground_station_ip}', f'port={video_port}'
            ]
    ...
    def start_streaming(self) -> bool:
        ...
        try:
            self.logger.info(
                f'Starting video stream to '
                f'{self.config["ground_station_ip"]}:{self.config["video_port"]}'
            )
    ...
    def monitor_stream(self) -> None:
        ...
        while self.streaming and not self.stop_event.is_set():
            if self.gstreamer_process:
                ...
                if poll is not None:
                    self.logger.error(
                        f'GStreamer process terminated with code {poll}'
                    )
